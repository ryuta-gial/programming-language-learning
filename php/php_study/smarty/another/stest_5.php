...
if(isset($_POST["send"]) === true){
	...
	$check = new check();
	$list = $check->check_post($name,$age,$sex,$language,$tmp_image,$dbc);
	//var_dump($db);
...
{ 
	...
	public function check_post($cname,$cage,$csex,$clanguage,$ctmp_image,$dbc)
	{
		$err_msg = array();
		...
		if(isset($err_msg['name']) && isset($err_msg['age']) && isset($err_msg['language']) ){
			...
			if($ctmp_image['error'] === 0 && $ctmp_image['size'] !==0 ){
				...
				if(is_uploaded_file($ctmp_image['tmp_name']) === true){
					...
					$err_msg['pic'] = mysqli_real_escape_string($dbc, $imgdat);
...
$smarty->assign('err_msg1', $list['name']);
$smarty->assign('err_msg2', $list['age']);
$smarty->assign('err_msg3', $list['language']);
$smarty->assign('err_msg4', $list['image']);
$smarty->assign('data', $data);

//テンプレートの設定
$smarty->display('/Applications/MAMP/htdocs/smarty/templates/stest_5.tpl');
